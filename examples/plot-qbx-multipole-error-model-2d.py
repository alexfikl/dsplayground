...
def compute_target_reconstruction_error(
        actx, interaction_mat, kernel, places, index_set, *,
        id_eps: float,
        auto_where: Optional[Any] = None,
        verbose: bool = True) -> float:
    proxy_mat = kernel(places, index_set=index_set, auto_where=auto_where)
...
def main(ctx_factory, visualize: bool = True) -> None:
    ...
    target_indices = make_block_index_from_array(
            [partition.block_indices(nblocks // 2)])

    from pytential.linalg.utils import MatrixBlockIndexRanges
    indices = MatrixBlockIndexRanges(target_indices, source_indices)
    ...
    nodes = np.stack(
            to_numpy(flatten(thaw(density_discr.nodes(), actx)), actx)
            )
    ...
    if visualize:
        ...
        ax.plot(source_nodes[0], source_nodes[1], "o")
        ax.plot(target_nodes[0], target_nodes[1], "o")
    ...
    kernel = partial(ds.evaluate_qbx, actx, kernel, context={})

    interaction_mat = kernel(places,
            index_set=indices,
